    ...
    public void onLoadFinished(@NonNull Loader <String>loader, String data) {
        ...
        try{
            ...
            for (int i=0; i<size;i++){
                ...
                JSONArray pointCoordinate = infos.getJSONArray("PointCoordinate");
                JSONObject cameraDetails = camera.getJSONObject(0);
                Double latitude = pointCoordinate.getDouble(0);
                Double longitude = pointCoordinate.getDouble(1);
                String description = cameraDetails.getString("Description");
                String imageURL = cameraDetails.getString("ImageUrl");

                //Log.d(LOG_TAG, Double.toString(rawLatitude));
                cameraName.add(description);
                cameraURL.add(imageURL);
                cameraLatitude.add(latitude);
                cameraLongitude.add(longitude);
            ...
            cameraData.putSerializable("latitudeArray", cameraLatitude);
            cameraData.putSerializable("longitudeArray", cameraLongitude);
